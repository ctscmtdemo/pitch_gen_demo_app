...
with st.container(border=True, key="oc",height=500):
    ...
    for mapping in mappings:
        if st.session_state.model_response["company_name"] == mapping["customer_name"]:
            st.session_state.outlines = mapping["outlines"]
            st.session_state.deck_url = mapping["deck_url"]
            break            
        elif st.session_state.model_response["industry"] == mapping["industry"] or st.session_state.model_response["solution"] == mapping["solution"]:
            if st.session_state.model_response["company_name"] == "":
                st.session_state.outlines = mapping["outlines"]
                st.session_state.deck_url = mapping["deck_url"]
                break
            elif st.session_state.model_response["company_name"] == mapping["customer_name"]:
                st.session_state.outlines = mapping["outlines"]
                st.session_state.deck_url = mapping["deck_url"]
                break
